...
void AStrategyMenuHUD::RebuildWidgets(bool bHotReload)
{
	...
	if (GEngine && GEngine->GameViewport)
	{
		...
		SAssignNew(MenuWidget, SStrategyMenuWidget).Cursor(EMouseCursor::Default).MenuHUD(MakeWeakObjectPtr(this));
		GVC->AddViewportWidgetContent(SNew(SWeakWidget).PossiblyNullContent(MenuWidget.ToSharedRef()));
	}
}
