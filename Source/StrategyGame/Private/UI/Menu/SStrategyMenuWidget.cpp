...
void SStrategyMenuWidget::Construct(const FArguments& InArgs)
{
	...
	ChildSlot
	[
		SNew(SDPIScaler).DPIScale(UIScale) // SDPIScaler
		[
			SNew(SOverlay)

			+ SOverlay::Slot().HAlign(HAlign_Center).VAlign(VAlign_Fill)
			[
				SNew(SScaleBox).Stretch(EStretch::ScaleToFitX).StretchDirection(EStretchDirection::UpOnly) // ScaleBox
				[
					SNew(SImage).Image(&MenuStyle->BackgroundBrush)
				]
			]
			...
			[
				...
				+ SVerticalBox::Slot().FillHeight(1.0f).Padding(TAttribute<FMargin>(this,&SStrategyMenuWidget::GetMenuOffset))
				[
					SNew(SBorder).BorderImage(FCoreStyle::Get().GetBrush("NoBorder")).DesiredSizeScale(this, &SStrategyMenuWidget::GetBottomScale).VAlign(VAlign_Top).HAlign(HAlign_Left).Clipping(EWidgetClipping::ClipToBounds)
					[
						SNew(SOverlay)

						+ SOverlay::Slot().HAlign(HAlign_Fill).VAlign(VAlign_Fill).Padding(FMargin(BgPadding))
						[
							SNew(SImage).Image(&MenuStyle->TileBackgroundBrush)
						]
						+ SOverlay::Slot().HAlign(HAlign_Fill).VAlign(VAlign_Fill)
						[
							SNew(SBorder).BorderImage(&MenuStyle->PopupBackgroundBrush).ColorAndOpacity(this, &SStrategyMenuWidget::GetBottomColor).VAlign(VAlign_Center).HAlign(HAlign_Center)
							[
								SNew(SOverlay)

								+ SOverlay::Slot().HAlign(HAlign_Fill).VAlign(VAlign_Fill).Padding(FMargin(MenuPaddingX,MenuPaddingY))
								[
									SNew(SHorizontalBox)

									+ SHorizontalBox::Slot().AutoWidth().Padding(TAttribute<FMargin>(this,&SStrategyMenuWidget::GetLeftMenuOffset))
									[
								...
								[
									SNew(SImage).Image(&MenuStyle->LeftDecoration)
								]
								+ SOverlay::Slot().HAlign(HAlign_Right).VAlign(VAlign_Center)
								[
									SNew(SImage).Image(&MenuStyle->RightDecoration)
								]
							]
						]
					]
			...
			+ SOverlay::Slot().HAlign(HAlign_Fill).VAlign(VAlign_Fill)
			[
				SNew(SCanvas).Visibility(EVisibility::HitTestInvisible)

				+SCanvas::Slot().Position(TAttribute<FVector2D>(this, &SStrategyMenuWidget::GetTopDecorPosition)).Size(TAttribute<FVector2D>(this, &SStrategyMenuWidget::GetTopDecorSize))
				[
					SAssignNew(TopDecorImage,SImage).Image(&MenuStyle->TopDecoration)
				]
			]
		]
	];
...
void SStrategyMenuWidget::BuildLeftPanel(bool bInGoingBack)
{
	...
	for(int32 i = 0; i < (*MyMenuHUD->CurrentMenu).Num(); ++i)
	{
		...
		TmpButton = SAssignNew((*MyMenuHUD->CurrentMenu)[i].Widget,SStrategyMenuItem).OnClicked(this, &SStrategyMenuWidget::ButtonClicked, i).Text((*MyMenuHUD->CurrentMenu)[i].Text);
		(*MyMenuHUD->CurrentMenu)[i].Widget->SetImage(MyMenuHUD->MenuButtonTexture);
		(*MyMenuHUD->CurrentMenu)[i].Widget->DeferredShow();
		LeftBox->AddSlot().HAlign(HAlign_Left).AutoHeight().Padding(FMargin(0,10))
		[
	...
	if (MenuWidgetAnimation.IsAtEnd())
	{
		if (bLeftMenuChanging)
		{
			if (LeftMenuWidgetAnimation.IsAtEnd())
			{
				...
				if (MyMenuHUD->NextMenu.IsValid() && MyMenuHUD->NextMenu->Num() > 0 && ((*MyMenuHUD->NextMenu))[0].SubMenu.IsValid() && ((*MyMenuHUD->NextMenu))[0].SubMenu->Num() > 0)
				{
...
FMargin SStrategyMenuWidget::GetMenuOffset() const
{
	...
	const FMargin Result     = FMargin(OffsetX + (CurrentViewportSize.X / Scale) - AnimProgress * (CurrentViewportSize.X / Scale), MenuOffsetY, 0, 0);
	return Result;
}
