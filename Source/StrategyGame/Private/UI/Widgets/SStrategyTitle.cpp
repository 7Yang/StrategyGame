...
void SStrategyTitle::Construct(const FArguments& InArgs)
{
	...
	[
		...
		[
			SNew(STextBlock).ShadowColorAndOpacity(this, &SStrategyTitle::GetTitleShadowColor).ShadowOffset(FIntPoint(-1,1)).ColorAndOpacity(this, &SStrategyTitle::GetTitleColor).Font(this, &SStrategyTitle::GetTitleFont).Text(this, &SStrategyTitle::GetTitleText)
		]
	];
}
...
FSlateFontInfo SStrategyTitle::GetTitleFont() const
{
	...
	FSlateFontInfo ResultFont = FSlateFontInfo(FPaths::ProjectContentDir() / TEXT("Slate/Fonts/Roboto-Regular.ttf"), FMath::TruncToInt(StartFontSize + AnimatedFontSize * AnimPercentage));
	return ResultFont;
}
...
void SStrategyTitle::ShowTitle(FText InTitleText)
{
	...
	GEngine->GameViewport->AddViewportWidgetContent(SAssignNew(TitleContainer, SWeakWidget).PossiblyNullContent(SharedThis(this)));
}
...
{
	if (!OwnerWorld.IsValid()) return 0.0f;
