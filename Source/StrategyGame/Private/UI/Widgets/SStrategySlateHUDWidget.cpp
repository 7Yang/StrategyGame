...
void SStrategySlateHUDWidget::Construct(const FArguments& InArgs)
{
	...
	[
		SNew(SDPIScaler).DPIScale(UIScale)
		[
			SNew(SOverlay)
			...
			[
				...
				+SCanvas::Slot().Position(TAttribute<FVector2D>(this,&SStrategySlateHUDWidget::GetActionsWidgetPos)).Size(FVector2D(600,800))
				[
					SAssignNew(ActionButtonsWidget,SStrategyActionGrid).OwnerHUD(OwnerHUD)
				]
			]
			+SOverlay::Slot().VAlign(VAlign_Bottom).HAlign(HAlign_Left).Padding(FMargin(MiniMapBorderMargin,0,0,MiniMapBorderMargin))
			[
				SNew(SBorder).BorderImage(&HUDStyle->MinimapFrameBrush).Padding(FMargin(0))
				[
					SNew(SBox).Padding(FMargin(MiniMapBorderMargin,MiniMapBorderMargin)).WidthOverride(this,&SStrategySlateHUDWidget::GetMiniMapWidth).HeightOverride(this,&SStrategySlateHUDWidget::GetMiniMapHeight)
					[
						SAssignNew(MiniMapWidget,SStrategyMiniMapWidget).OwnerHUD(OwnerHUD)
					]
				]
			]
			+SOverlay::Slot().VAlign(VAlign_Top).HAlign(HAlign_Center)
			[
				SNew(SBorder).BorderImage(&HUDStyle->ResourcesBackgroundBrush).Padding(FMargin(30.0f,10.0f))
				[
					SNew(SBox).HAlign(HAlign_Center).VAlign(VAlign_Center).WidthOverride(200).HeightOverride(60)
					[
						SNew(SHorizontalBox).Visibility(this, &SStrategySlateHUDWidget::GetResourcesVisibility)

						+SHorizontalBox::Slot().AutoWidth()
						[
							SNew(STextBlock).TextStyle(FStrategyStyle::Get(), "StrategyGame.ResourcesTextStyle").Text(this, &SStrategySlateHUDWidget::GetResourcesAmount)
						]
						+SHorizontalBox::Slot().AutoWidth()
						[
							SNew(SVerticalBox)
							...
							[
								...
								[
									SNew(SImage).Image(&HUDStyle->ResourcesImage)
								]
							]
						]
					]
			...
			[
				SNew(STextBlock).TextStyle(FStrategyStyle::Get(), "StrategyGame.ResourcesTextStyle").Text(this, &SStrategySlateHUDWidget::GetGameTime)
			]
			/* Result screen { */
			...
			[
				...
				+SOverlay::Slot().VAlign(VAlign_Center).HAlign(HAlign_Center)
				[
					SNew(SImage).Image(this, &SStrategySlateHUDWidget::GetGameResultImage).Visibility(this, &SStrategySlateHUDWidget::GetResultScreenVisibility)
				]
				+SOverlay::Slot().VAlign(VAlign_Bottom).HAlign(HAlign_Center)
				[
					SNew(SBox).HAlign(HAlign_Center).VAlign(VAlign_Center).WidthOverride(675).HeightOverride(310)
					[
						SNew(STextBlock).Visibility(this, &SStrategySlateHUDWidget::GetResultScreenVisibility).ShadowColorAndOpacity(FLinearColor::Black).ColorAndOpacity(this, &SStrategySlateHUDWidget::GetGameResultColor).ShadowOffset(FIntPoint(-1,1)).Font(this, &SStrategySlateHUDWidget::GetGameResultFont).Text(this, &SStrategySlateHUDWidget::GetGameResultText)
					]
				]
			]
			...
			+SOverlay::Slot().VAlign(VAlign_Fill).HAlign(HAlign_Fill)
			[
				SNew(SBorder).Visibility(this, &SStrategySlateHUDWidget::GetPauseMenuBgVisibility).VAlign(VAlign_Fill).HAlign(HAlign_Fill).BorderImage(FCoreStyle::Get().GetBrush("BlackBrush")).BorderBackgroundColor(this, &SStrategySlateHUDWidget::GetOverlayColor)
				[
					SNew(SOverlay)
					...
					[
                        ...
                        +SVerticalBox::Slot()
						[
							SAssignNew(PauseMenuButtons[ButtonIndex++], SStrategyButtonWidget).OwnerHUD(OwnerHUD).Visibility(EVisibility::Visible).TextFont(FStrategyStyle::Get().GetFontStyle(TEXT("StrategyGame.MenuFont"))).TextVAlign(EVerticalAlignment::VAlign_Center).TextMargin(FMargin(0)).ButtonText(NSLOCTEXT("SStrategySlateHUDWidget", "MainMenu", "Main Menu")).OnClicked(this, &SStrategySlateHUDWidget::OnReturnToMainMenu)
						]
					]
					
				]
			]
			+SOverlay::Slot().VAlign(VAlign_Bottom).HAlign(HAlign_Right).Padding(FMargin(0,0,20,20))
			[
				SAssignNew(PauseButton,SStrategyButtonWidget).OwnerHUD(OwnerHUD).Visibility(EVisibility::Visible).OnClicked(this, &SStrategySlateHUDWidget::TogglePauseMenu).ButtonText(FText::GetEmpty())
			]
		]
	];

	{
		...
		MenuBox->AddSlot()
		[
			SAssignNew(PauseMenuButtons[ButtonIndex++], SStrategyButtonWidget).OwnerHUD(OwnerHUD).Visibility(EVisibility::Visible).TextFont(FStrategyStyle::Get().GetFontStyle(TEXT("StrategyGame.MenuFont"))).TextVAlign(EVerticalAlignment::VAlign_Center).TextMargin(FMargin(0)).ButtonText(NSLOCTEXT("SStrategySlateHUDWidget", "CheatGold", "Cheat-Gold")).OnClicked(this, &SStrategySlateHUDWidget::OnCheatAddGold)
		];	
	}
	...
	if (SupportsQuitButton)
	{
		MenuBox->AddSlot()
		[
			SAssignNew(PauseMenuButtons[ButtonIndex++], SStrategyButtonWidget).OwnerHUD(OwnerHUD).Visibility(EVisibility::Visible).TextFont(FStrategyStyle::Get().GetFontStyle(TEXT("StrategyGame.MenuFont"))).TextVAlign(EVerticalAlignment::VAlign_Center).TextMargin(FMargin(0)).ButtonText(NSLOCTEXT("SStrategySlateHUDWidget", "Exit", "Exit")).OnClicked(this, &SStrategySlateHUDWidget::OnExitGame)
		];
	}
}
