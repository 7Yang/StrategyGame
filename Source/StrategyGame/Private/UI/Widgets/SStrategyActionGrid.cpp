...
void SStrategyActionGrid::Construct(const FArguments& InArgs)
{
	...
	[
		...
		[
			SAssignNew(ActionGrid, SGridPanel) // SGridPanel
		]
	];
	...
	for (int32 i = 0; i < GridRows; i++)
	{
		...
		{
			ActionGrid->AddSlot(j, i).Padding(TAttribute<FMargin>::Create(TAttribute<FMargin>::FGetter::CreateSP(this, &SStrategyActionGrid::GetActionPadding, i * GridRows + j)))
			[
				SAssignNew(ActionButtons[i*GridRows+j]->Widget, SStrategyButtonWidget).OwnerHUD(OwnerHUD).ToolTip(SNew(SToolTip).Text(this, &SStrategyActionGrid::GetTooltip,i*GridRows+j)).IsEnabled(this, &SStrategyActionGrid::GetEnabled, i*GridRows+j).Visibility(this, &SStrategyActionGrid::GetActionVisibility, i*GridRows+j).OnClicked(this, &SStrategyActionGrid::TriggerAction, i*GridRows+j).ButtonText(this, &SStrategyActionGrid::GetActionCostText, i*GridRows+j).CenterText(this, &SStrategyActionGrid::GetActionText, i*GridRows+j).CornerText(this,&SStrategyActionGrid::GetActionQueueText, i*GridRows+j).CoinIconVisible(this, &SStrategyActionGrid::GetCoinIconVisibility, i*GridRows+j)
			];
		}
	}
}
...
FMargin SStrategyActionGrid::GetActionPadding(int32 idx) const
{
	...
	static const FMargin ActionButtonsMargins[9] = 
    { 
...
{
	if (ActionButtons[idx]->Widget.IsValid() && ActionButtons[idx]->Data.TriggerDelegate.IsBound() && ActionButtons[idx]->Data.TriggerDelegate.Execute())
	{
