...
/**
 * This is the main drawing pump.  
 * It will determine which hud we need to draw (Game or PostGame).  
 * Any drawing that should occur regardless of the game state should go here.
 */
...
void AStrategyHUD::BuildMenuWidgets()
{
	...
	if (!MyHUDMenuWidget.IsValid())
	{
		...
		if (PCOwner)
		{
			SAssignNew(MyHUDMenuWidget, SStrategySlateHUDWidget).OwnerHUD(this);
			if (MyHUDMenuWidget.IsValid())
			{
				GEngine->GameViewport->AddViewportWidgetContent(SNew(SWeakWidget).PossiblyNullContent(MyHUDMenuWidget.ToSharedRef()));
...
{
	if (MyHUDMenuWidget.IsValid() && MyHUDMenuWidget->ActionButtonsWidget.IsValid() && MyHUDMenuWidget->ActionButtonsWidget->ActionButtons.IsValidIndex(Index))
	{
...
{
	if (MyHUDMenuWidget.IsValid() && MyHUDMenuWidget->ActionButtonsWidget.IsValid())
	{
...
void AStrategyHUD::DrawLives() const
{
	...
	FCanvasTileItem TileItem(FVector2D(0,0), LivesTexture->GetResource(), FVector2D(TextureDrawWidth, TextureDrawHeight), FLinearColor::White);
	TileItem.BlendMode = SE_BLEND_Translucent;
...
void AStrategyHUD::DrawHealthBar(AActor* ForActor, float HealthPercentage, int32 BarHeight, int32 OffsetY) const
{
	...
	FCanvasTileItem TileItem( FVector2D( X, Y ), HealthBarTexture->GetResource(), FVector2D(HealthBarLength * HealthPercentage, BarHeight), FLinearColor::White);
	TileItem.BlendMode = SE_BLEND_Translucent;
	...
	TileItem.Texture  = BarFillTexture->GetResource();
...
{
    return MyHUDMenuWidget.IsValid() ? MyHUDMenuWidget->IsPauseMenuUp() : false;
}
