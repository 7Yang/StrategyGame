...
AStrategyProjectile* UStrategyGameBlueprintLibrary::SpawnProjectile(UObject* WorldContextObject, UBlueprint* ProjectileBlueprint,
	const FVector& SpawnLocation, const FVector& ShootDirection, TEnumAsByte<EStrategyTeam::Type> OwnerTeam, int32 ImpactDamage, float LifeSpan, AStrategyBuilding* InOwner)
{
	return SpawnProjectileFromClass(WorldContextObject, ProjectileBlueprint ? *ProjectileBlueprint->GeneratedClass : nullptr, SpawnLocation, ShootDirection, OwnerTeam, ImpactDamage, LifeSpan, InOwner);
}

void UStrategyGameBlueprintLibrary::GiveBuff(AStrategyChar* InChar, int32 AttackMin, int32 AttackMax, int32 DamageReduction, int32 MaxHealthBonus, int32 HealthRegen, float Speed, float Duration, bool bInfiniteDuration, int32 AttackDistance)
{
	FBuffData NewBuff;
...
{
    return InMinionChar ? InMinionChar->Destroy() : false;
}
...
void UStrategyGameBlueprintLibrary::ShowWaveTitle(UObject* WorldContextObject, int32 WaveNumber)
{
	...
	if( MyGameMode != nullptr )
	{
		...
		SAssignNew(MyGameMode->StrategyTitle, SStrategyTitle).Visibility(EVisibility::HitTestInvisible).OwnerWorld(MyWorld);
