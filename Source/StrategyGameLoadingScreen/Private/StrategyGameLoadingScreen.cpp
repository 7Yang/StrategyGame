...
struct FStrategyGameLoadingScreenBrush : public FSlateDynamicImageBrush, public FGCObject
{
    ...
    {
        return FString(TEXT("StrategyGameLoadingScreenBrush"));
    }
};
	...
	void Construct(const FArguments& InArgs)
	{
		...
		[
			...
			[
				...
				+SVerticalBox::Slot().VAlign(VAlign_Bottom).HAlign(HAlign_Right).Padding(FMargin(10.0f))
				[
					SNew(SThrobber).Visibility(this, &SStrategyLoadingScreen::GetLoadIndicatorVisibility)
				]
			]
		];
	}
